...
#import "AgoraNameViewController.h"
#import "AgoraUserProfileManager.h"

@interface AgoraNameViewController ()<UITextFieldDelegate>

@property (nonatomic, strong) UITextField *nameTextField;
@end

@implementation AgoraNameViewController
...
- (UITextField *)nameTextField
{
    if (!_nameTextField) {
        ...
        NSString *user = [[AgoraUserProfileManager sharedInstance] getNickNameWithUsername:[[AgoraChatClient sharedClient] currentUsername]];
...
- (void)updateMyName:(NSString *)newName
{
    ...
    {
        _myName = newName;
        [[AgoraUserProfileManager sharedInstance] updateUserProfileInBackground:@{kPARSE_HXUSER_NICKNAME:newName} completion:^(BOOL success, NSError *error) {}];
        [[AgoraChatClient sharedClient] updatePushNotifiationDisplayName:newName completion:^(NSString *aDisplayName, AgoraError *aError) {}];
        if (self.callBack) {
