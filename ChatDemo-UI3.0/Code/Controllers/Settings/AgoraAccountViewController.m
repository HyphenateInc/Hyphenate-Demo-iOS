...
@interface AgoraAccountViewController ()<UIImagePickerControllerDelegate, UINavigationControllerDelegate>
...
- (void)viewDidLoad
{
    ...
    [[AgoraUserProfileManager sharedInstance] loadUserProfileInBackground:@[currentUser] saveToLoacal:YES completion:^(BOOL success, NSError *error) {
        if (!error && success) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        ...
        AgoraNameViewController *name = [[AgoraNameViewController alloc] init];
