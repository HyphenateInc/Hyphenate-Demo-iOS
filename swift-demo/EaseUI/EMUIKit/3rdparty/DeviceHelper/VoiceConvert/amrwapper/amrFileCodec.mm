...
// From the WAVE file, Skip the WAVE header to PCM audio data
...
static void WriteWAVEFileHeader(FILE* fpwave, int nFrame)
{
	...
	wfx.nFormatTag = 1;
	wfx.nChannels = 1; // Single audio channel
	wfx.nSamplesPerSec = 8000; // 8khz
	wfx.nAvgBytesPerSec = 16000;
	wfx.nBlockAlign = 2;
	wfx.nBitsPerSample = 16; // 16 digit
...
{
	size_t bytes = 0;
	unsigned char frameHeader; // Frame header
	
	//memset(frameBuffer, 0, sizeof(frameBuffer));
	
    // Read the frame header
    // If it is a bad frame(not a standard frame)，continue for the next bit
	while(1)
	{
