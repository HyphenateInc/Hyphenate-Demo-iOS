/*!
 *  \~chinese
 *  @header AgoraGroupManagerDelegate.h
 ...
 */
typedef enum{
    AgoraGroupLeaveReasonBeRemoved = 0,    /*! \~chinese 被移除 \~english Removed by owner */
    AgoraGroupLeaveReasonUserLeave,        /*! \~chinese 自己主动离开 \~english User leave the group */
    AgoraGroupLeaveReasonDestroyed,        /*! \~chinese 群组销毁 \~english Group has been destroyed */
}AgoraGroupLeaveReason;

@class AgoraGroup;
@class AgoraGroupSharedFile;
 ...
 *  Delegate
 */
@protocol AgoraGroupManagerDelegate <NSObject>
...
/*!
 *  \~chinese
 *  SDK自动同意了用户A的加B入群邀请后，用户B接收到该回调，需要设置AgoraOptions的isAutoAcceptGroupInvitation为YES
 *
 *  @param aGroup    群组实例
 *  @param aInviter  邀请者
 *  @param aMessage  邀请消息
...
/*!
 *  \~chinese
 *  群组的群主收到用户的入群申请，群的类型是AgoraGroupStylePublicJoinNeedApproval
 *
 *  @param aGroup     群组实例
...
/*!
 *  \~chinese
 *  群主拒绝用户A的入群申请后，用户A会接收到该回调，群的类型是AgoraGroupStylePublicJoinNeedApproval
 *
 *  @param aGroupId    群组ID
 *  @param aReason     拒绝理由
 *
...
/*!
 *  \~chinese
 *  群主同意用户A的入群申请后，用户A会接收到该回调，群的类型是AgoraGroupStylePublicJoinNeedApproval
 *
 ...
 *  群组列表发生变化
 *
 *  @param aGroupList  群组列表<AgoraGroup>
 *
 *  \~english
 *  Group List updated
 *
 *  @param aGroupList  Group list<AgoraGroup>
 */
 ...
 *  Set AgoraOptions's isAutoAcceptGroupInvitation property to YES for this delegate method
 *
 *  @param aGroup    Group instance
 *  @param aInviter  Inviter
 *  @param aMessage  Invite message
...
- (void)didReceiveLeavedGroup:(AgoraGroup *)aGroup
                       reason:(AgoraGroupLeaveReason)aReason __deprecated_msg("Use -didLeaveGroup:reason:");
 ...
 *  @param aGroup   Group instance
 */
- (void)didReceiveAcceptedJoinGroup:(AgoraGroup *)aGroup __deprecated_msg("Use -joinGroupRequestDidApprove:");
